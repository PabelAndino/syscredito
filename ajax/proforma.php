...
switch ($_GET["op"]){
    case 'guardaryeditar':
        ...
        if (empty($idventa)){
            ...
            $rspta=$proforma->insertar($idcliente,$idusuario,$tipo_comprobante,$serie_comprobante,$num_comprobante,$fecha_hora,$impuesto,$total_venta,$tipoventa,
    ...
    case 'mostrar':
        ...
        $rspta=$proforma->mostrar($idventa);
    ...
    case 'mostrarNumero': //MUSTRA EL NUMERO DE SERIE AUTOINCREMENTADO
        ...
        $rspta=$proforma->sumarNumeroFactura();
        //Codificar el resultado utilizando json
    ...
    case 'listar':
        ...
        while ($reg=$rspta->fetch_object()){
                ...
                $urlFACT='../reportes/FacturaRepProforma.php?id=';


            $data[]=array(
                "0"=>(($reg->estado=='Aceptado')?'<button class="btn btn-warning" onclick="mostrar('.$reg->idproforma.')"><i class="fa fa-eye"></i></button>'.
                   ...
                   // '<a target="_blank" href="'.$urlFACT.$reg->idventa.'"> <button class="btn btn-info"><i class="fa fa-file"></i></button></a>',
                    ...
                    '<a target="_blank" href="'.$urlFACT.$reg->idproforma.'"> <button class="btn btn-info"><i class="fa fa-file"></i></button></a>',
                "1"=>$reg->fecha,
                "2"=>$reg->cliente,
                "3"=>$reg->usuario,
    ...
    case 'listarArticulosVenta':
        ...
        while ($reg=$rspta->fetch_object()){
            $data[]=array(
                "0"=>'<button class="btn btn-warning" id="botonon" onclick="agregarDetalle('.$reg->stock.',\''.$reg->idarticulo.'\', \''.$reg->nombre.'\', \''.$reg->precio_venta.'\')" ><span class="fa fa-plus"></span></button>',
                "1"=>$reg->nombre,
                "2"=>$reg->categoria,
                "3"=>$reg->codigo,
                "4"=>$reg->stock,
