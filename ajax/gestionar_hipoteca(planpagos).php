                ...
                if($mes_aniodesembolso < $mes_aniopago){ //verifica que no este en el mismo mes
                   ...
                   // echo ' *** Ultimo mes y dias: ' ;
                    ...
                    foreach ($dias as $values){
                        ...
                        $diasTotales_delmes= $values; //en una primera instancia values son todos los dias del mes que es guardado en diasTotales-delmes
                      //  $values = round((($values * $interes)/100),2); //luego lo saco el porcentaje del interes, ahora values contienen el interes
                        if($contador==1){
                            //verifica si es el primer elemento para asignar a la variable w la cantidad de dias que se tienen que tomar en la primera fecha
                            $diasTotales_delmes = ($diasTotales_delmes - $diadesembolso); //Guarda la cantidad de dias que hay en el mes del desembolso
                            $dias_conPorcentajes =  $diasTotales_delmes;

                        }else if ($values !== count($dias) -1){
                            //verifica el ultimo item del array dias,
                            ...
                            $dias_conPorcentajes = $diasTotales_delmes; //la cantidad de dias que hay en la fecha del pago

                        }
                        //luego x pasa a hacer la multiplicacion de de v que ahora tiene el porcentaje calculado y w que tiene los dias del mes donde V es el porcentaje por dia
                    ...
                    $sumaDiasdelmes = array_sum($diascaculados);

                    $interesDiario=  $interes_calculado / 30;
                    $interes_calculadoPrimerFecha = $sumaDiasdelmes * $interesDiario;//$interesDiario * $sumaDiasdelmes;
                    $interes_calculado = $interes_calculadoPrimerFecha;

                    $mantenimiento_diario = $mantValor / 30;
                    $mantenimiento_calculadoPrimerFecha = $sumaDiasdelmes * $mantenimiento_diario;
                    $mantValor = $mantenimiento_calculadoPrimerFecha;
                    // echo $r;
                  ...
                  //  echo '/Dias calculados /';
                    ...
                    $interesDiario=  $interes_calculado / 30;
                    $interes_calculadoPrimerFecha = $r * $interesDiario;//$interesDiario * $sumaDiasdelmes;
                    $interes_calculado = $interes_calculadoPrimerFecha;

                    $mantenimiento_diario = $mantValor / 30;
                    $mantenimiento_calculadoPrimerFecha = $r * $mantenimiento_diario;
                    $mantValor = $mantenimiento_calculadoPrimerFecha;

                    // $interesDiario = $interes_calculado / $dias_delmes;
